...
_POPULATION_PATH = "raw_data/uk_lad_population_estimates.xls"
_LTLA_TO_NHS_PATH = "raw_data/ltla_to_nhs.csv"
_ENGLAND_META_AREAS = "raw_data/england_meta_areas.csv"
_SCOTLAND_HEALTH_BOARDS = "raw_data/nhs_scotland_health_boards.csv"
_TOPO_JSON_URL = "raw_data/uk_lad_boundaries.json"
...
if __name__ == "__main__":
    ...
    lad_population_df = population_df[
        [
            "Code",
            "Name",
    ...
    lad_mapping_df = (
        pd.read_csv(_LTLA_TO_NHS_PATH, usecols=["LAU118NM", "NHS Region"])
        .rename(columns={"LAU118NM": "AREA", "NHS Region": "NHS_Region"})
        .set_index("AREA")
    )
    ...
    meta_df = pd.DataFrame(
        [
            (
                l["properties"]["lad20cd"],
                l["properties"]["lad20nm"],
                l["properties"]["lat"],
                l["properties"]["long"],
            )
            for l in local_authorities_official["objects"][
                "Local_Authority_Districts__May_2020__Boundaries_UK_BFC"
            ]["geometries"]
        ],
    ...
    scotland_map = pd.read_csv(_SCOTLAND_HEALTH_BOARDS).set_index("area")
    ...
    england_map = pd.read_csv(_ENGLAND_META_AREAS).set_index("area")
    ...
    print("Wrote ", os.path.join(args.output_dir, "metadata.csv"))
