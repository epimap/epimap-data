...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-dir", default="processed_data/comparisons/")
    args = parser.parse_args()

    page = requests.get(
        "https://raw.githubusercontent.com/joshuablake/public-RTM-reports/master/2020-11-11.html"
    )
    ...
    df = pd.DataFrame(columns=["NHS Region", "Date"] + intervals).set_index(
        ["NHS Region", "Date"]
    )
    ...
    df.to_csv(
        os.path.join(
            args.output_dir, "MRC-BSU-Cambridge-nowcasting-and-forecasting.csv"
        ),
    )
